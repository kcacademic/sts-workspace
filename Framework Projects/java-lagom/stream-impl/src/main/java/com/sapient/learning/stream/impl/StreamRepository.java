...
public class StreamRepository {
	...
	// Will return the session when the Cassandra tables have been successfully
	// created
	...
	private CompletionStage<CassandraSession> session() {
		// If there's no initialised session, or if the initialised session future
		// completed
		// with an exception, then reinitialise the session and attempt to create the
		// tables
	...
	public CompletionStage<Done> updateMessage(String name, String message) {
		return session().thenCompose(session -> session
				.executeWrite("INSERT INTO greeting_message (name, message) VALUES (?, ?)", name, message));
	}

	public CompletionStage<Optional<String>> getMessage(String name) {
		return session()
				.thenCompose(session -> session.selectOne("SELECT message FROM greeting_message WHERE name = ?", name))
