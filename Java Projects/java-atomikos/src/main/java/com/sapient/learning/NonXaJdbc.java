...
public class NonXaJdbc {
	...
	public static void createAccountsTableIfNecessary() throws Exception {
		boolean error = false;
	...
	private static Connection getConnectionWithinNewTransaction() throws Exception {
	...
	private static void closeConnectionAndEndTransaction(Connection conn, boolean rollback) throws Exception {
		if (conn != null)
			conn.close();
	...
	public static String getOwner(int account) throws Exception {
		...
		try {
			conn = getConnectionWithinNewTransaction();
			Statement s = conn.createStatement();
			String query = "select owner from Accounts where account='account" + account + "'";
			ResultSet rs = s.executeQuery(query);
			if (rs == null || !rs.next())
				throw new Exception("Account not found: " + account);
			res = rs.getString(1);
